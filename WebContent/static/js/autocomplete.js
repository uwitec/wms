...
$.Autocompleter = function(input, options) {
	...
	// 当按RETURN键(默认为回车)时，在opera浏览器中阻止表单提交
	...
	}).bind((isOpera ? "keypress" : "keydown") + ".autocomplete", function(event) {
	...
	}).bind("showSelect", function(event) {
		event.preventDefault();
		if (!select.visible()) 
			onChange(0, true);
	});
		...
		if (options.dataType && options.dataType.toLowerCase() == 'json' ) {
			...
			for (var i=0; i < data.length; i++) {
				...
				parsed[parsed.length] = {
					data: data[i],
					value: data[i][props[0]],
					result: options.formatResult && options.formatResult(data[i], data[i][props[0]]) || data[i][props[0]]
				};
			}				
		} else {
			var rows = data.split("\n");
