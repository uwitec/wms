...
public class WorkerUtil {
     ...
     * @return
     */
    public static Map<String, Date> checkValidityTime(String str,Integer month,String validity_unit,Integer warranty_warning_days) {
    	Date dt =null;
    	Map<String, Date>  resultMap  = new HashMap<String, Date>();
		try {
			dt = datetimeFormat.parse(str);
		
        Calendar validityRightNow = Calendar.getInstance();
        validityRightNow.setTime(dt);
        Calendar ExpiredrightNow = Calendar.getInstance();
        ExpiredrightNow.setTime(dt);
        Calendar oneOfFourthExpiredrightNow = Calendar.getInstance();
        oneOfFourthExpiredrightNow.setTime(dt);
        Calendar oneOfThreeExpiredrightNow = Calendar.getInstance();
        oneOfThreeExpiredrightNow.setTime(dt);
        Calendar oneMonthExpiredrightNow = Calendar.getInstance();
        oneMonthExpiredrightNow.setTime(dt);
        //分别计算已过期,临期1/4,临期1/3,临期1个月内 的时间

        if("MONTH".equalsIgnoreCase(validity_unit)){
        	ExpiredrightNow.add(Calendar.MONTH,month);
        	oneOfFourthExpiredrightNow.add(Calendar.MONTH,month);//临期1/4
        	oneOfFourthExpiredrightNow.add(Calendar.DAY_OF_MONTH,-month*30/4);//临期1/4
        	oneOfThreeExpiredrightNow.add(Calendar.MONTH,month);//临期1/3
        	oneOfThreeExpiredrightNow.add(Calendar.DAY_OF_MONTH,-month*30/3);//临期1/3
        	oneMonthExpiredrightNow.add(Calendar.MONTH,month-1);//临期1个月内
        	//oneMonthExpiredrightNow.add(Calendar.DAY_OF_MONTH,-30);//临期1个月内
        	resultMap.put("validityDate", validityRightNow.getTime());
        	resultMap.put("dateExpired", ExpiredrightNow.getTime());
        	resultMap.put("dateoneOfFourthExpired", oneOfFourthExpiredrightNow.getTime());
        	resultMap.put("dateoneOfThreeExpiredrightNow", oneOfThreeExpiredrightNow.getTime());
        	resultMap.put("dateoneMonthExpired", oneMonthExpiredrightNow.getTime());
        }else if ("DAY".equalsIgnoreCase(validity_unit)) {
        	ExpiredrightNow.add(Calendar.DAY_OF_MONTH,month);
        	oneOfFourthExpiredrightNow.add(Calendar.DAY_OF_MONTH,month-month/4);//临期1/4
        	oneOfThreeExpiredrightNow.add(Calendar.DAY_OF_MONTH,month-month/3);//临期1/3
        	oneMonthExpiredrightNow.add(Calendar.DAY_OF_MONTH,month-30);//临期1个月内
        	resultMap.put("validityDate", validityRightNow.getTime());
        	resultMap.put("dateExpired", ExpiredrightNow.getTime());
        	resultMap.put("dateoneOfFourthExpired", oneOfFourthExpiredrightNow.getTime());
        	resultMap.put("dateoneOfThreeExpiredrightNow", oneOfThreeExpiredrightNow.getTime());
...
//    }
    ...
    public static String generatorSequence() {
        return SequenceUtil.getInstance().getNextKeyValue(SequenceUtil.KEY_NAME_TAGCODE,true).toString();
    }
    
    public static String generatorSequence(String keyName,String type,Boolean flag) {
    	String code = SequenceUtil.getInstance().getNextKeyValue(keyName,flag).toString();
    	
    	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String f = "%0" + 8 + "d";
		
		String key = type+sdf.format((new Date()))+String.format(f, Integer.parseInt(code));
    	return key;
    }
	...
	public static String transferLocationBarcode(String str){
	...
	public static String addLocationBarcode(String str){
	...
	public static String minusComma(String str){
		char[]array = str.toCharArray();
		String temp ="";
...
//		}
		...
		String ip = req.getHeader("remoteip");  
